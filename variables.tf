...
variable "mgmt_sub_id" {
  type        = string
  description = "Sub ID for Management Resources"
}

variable "connectivity_sub_id" {
  type        = string
  description = "Sub ID for Connectivity Resources"
}

variable "security_sub_id" {
  type        = string
  description = "Sub ID for Security Resources"
}
...
variable "amba_settings" {
  description = "Settings required for AMBA integration with ALZ"
  type = object({
    location                                                  = string
    resource_group_name                                       = string
    user_assigned_managed_identity_name                       = string
    bring_your_own_user_assigned_managed_identity             = bool
    bring_your_own_user_assigned_managed_identity_resource_id = string
    amba_disable_tag_name                                     = string
    amba_disable_tag_values                                   = list(string)
    amba_action_group_email                                   = list(string)
  })
}

variable "management_group_pim_roles" {
  type = map(object({
    management_group_name = string
    role_definition_name  = string
    approver_group_key    = optional(string, null)
    condition_template    = optional(string, null)
  }))
  default = {}
}

variable "management_group_pim_approver_groups" {
  type    = list(string)
  default = []
}
